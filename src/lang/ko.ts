...
export const languageKorean = {
    pageFold: {
    "title": "PDF",
    "modelIdRequired": "Model ID를 입력하세요.",
    "requestAborted": "요청이 중단되었습니다.",
    "noPdfMessages": "PDF로 묶을 일반 대화가 없습니다. 이 도구 요청은 PDF 사용을 끄고 실행하세요.",
    "generating": "PDF 생성 중",
    "preparing": "PDF 전송 준비",
    "waiting": "응답 기다리는 중",
    "completed": "완료",
    "statsRestart": "서버를 다시 시작하면 PDF 통계가 활성화됩니다.",
    "statsLoadFailed": "PDF 통계를 불러오지 못했습니다. ({0})",
    "badgeInput": "입력 {0}",
    "badgeSaved": "예상 {0} 토큰 절약",
    "enable": "PDF 사용 (컨텍스트 캐싱과 동시 사용 불가)",
    "help": "대화와 프롬프트를 PDF로 묶어 현재 모델 설정으로 전달합니다.",
    "ineligible": "Model ID에 gemini가 포함된 모델에서 사용할 수 있습니다.",
    "settings": "PDF 설정",
    "mode": "패키징 모드",
    "maximum": "절약 — 전체 대화 PDF",
    "balanced": "안정 — 시스템 지시문 유지",
    "font": "PDF 글자 크기 (pt)",
    "merge": "연속 역할 병합",
    "mergeHelp": "연속된 system·user 메시지만 병합합니다. 이미지와 도구 기록은 별도로 유지합니다.",
    "cacheHelp": "PDF 사용 중에는 기존 컨텍스트 캐싱을 적용하지 않습니다.",
    "usage": "PDF 사용량",
    "openStats": "상세 통계",
    "priceSettings": "단가 설정 (선택)",
    "price": "입력 단가 (USD / 100만 토큰)",
    "priceHelp": "지원 게이트웨이는 단가를 자동 조회하며, 입력한 단가를 우선합니다. 비워도 토큰 절약량을 확인할 수 있습니다. 절약액은 추정값입니다.",
    "unknownPrice": "단가 미설정",
    "statsTitle": "PDF 사용량 및 요청 기록",
    "statsHelp": "프리셋별·모델별 PDF 사용량과 예상 절약액을 확인합니다.",
    "empty": "아직 PDF 요청 기록이 없습니다.",
    "loading": "PDF 사용량을 불러오는 중…",
    "refresh": "새로고침",
    "allPresets": "전체 PDF 요청",
    ...
    "export": "로그 JSON 내보내기",
    "reset": "PDF 기록 초기화",
    "exportConfirm": "저장된 전체 PDF 요청 로그에는 대화와 프롬프트가 포함됩니다. JSON으로 내보낼까요?",
    "resetConfirm": "전체 PDF 통계와 저장된 PDF 요청 로그를 초기화할까요? 조회 필터와 관계없이 적용되며, PDF가 아닌 요청 기록은 유지됩니다.",
    "comparisonHelp": "변환 전은 PageFold 원본의 문자 기반 추정, 변환 후는 API 사용량입니다. 이미지·도구가 섞여 비교하기 어려운 요청은 절약량에서 제외됩니다.",
    "userRequests": "사용자 요청",
    "requests": "API 시도",
    "successes": "성공",
    "pages": "PDF 페이지",
    ...
    "comparable": "비교 가능 요청",
    "cacheHits": "PDF 재사용",
    "inputKnown": "입력 사용량 확인",
    "outputKnown": "출력 사용량 확인",
    "failedInput": "실패·중단 입력 토큰",
    "byModel": "모델별 통계",
    "daily": "일별 통계",
    "recent": "최근 요청",
    ...
    "aborted": "중단",
    "failed": "실패",
    "detailTitle": "PDF 요청 상세",
    "requestBody": "요청 본문",
    "pdfBody": "PDF 내용",
    "responseBody": "응답 본문",
    "metadata": "사용량·설정",
    "missingBody": "상세 로그 저장이 꺼져 있거나 기록이 삭제되었습니다."
},
  cbsEditor: {
  ...
  chatModuleActivation: {
    ...
    description: "전체는 모든 캐릭터의 모든 챗에, 챗은 현재 캐릭터의 기존 챗과 새 챗에 항상 적용됩니다. 둘 중 하나라도 켜져 있으면 활성화됩니다.",
    selectDescription: "폴더에서 모듈을 찾아 선택하세요.",
    globalHint: "모든 캐릭터의 모든 챗에서 항상 활성화합니다.",
    chatHint: "현재 캐릭터의 모든 기존 챗과 앞으로 만들 새 챗에서 항상 활성화합니다.",
    noChat: "캐릭터를 선택한 뒤 사용할 수 있습니다.",
  ...
  v2ImportTitle: "V2 항목 가져오기",
  v2ImportDesc:
    "다른 RisuBard V2 세이브 폴더에서 선택한 캐릭터, 모듈, 페르소나, 프롬프트, 로어북만 현재 세이브에 안전하게 합칩니다. 원본 폴더는 변경하지 않으며 실패하면 현재 세이브도 그대로 유지됩니다.",
  v2ImportSourcePath: "원본 V2 세이브 폴더",
  v2ImportSourcePlaceholder: "예: E:\\Risuwork\\RisuBard-workspace\\other-save",
  v2ImportPreview: "항목 확인",
  v2ImportChecking: "폴더 확인 중...",
  v2ImportSelectAll: "모두 선택",
  v2ImportClearSelection: "선택 해제",
  v2ImportIncludeDependencies: "연결된 항목도 함께 가져오기",
  v2ImportDependencyHint: "캐릭터가 사용하는 모듈·페르소나 등이 빠지지 않도록 함께 선택합니다.",
  v2ImportEmpty: "가져올 수 있는 항목이 없습니다.",
  v2ImportAction: "선택 항목 가져오기",
  v2ImportProgress: (percent: number) => `가져오는 중... ${percent}%`,
  v2ImportConfirm: (count: number) =>
    `${count.toLocaleString()}개 항목을 현재 세이브에 합칩니다. 같은 ID는 새 ID로 바꾸고 같은 이름의 폴더에는 번호를 붙입니다. 원본 폴더는 변경하지 않습니다. 계속할까요?`,
  v2ImportSuccess: (count: number) => `${count.toLocaleString()}개 항목을 가져왔습니다. 새 데이터를 다시 불러옵니다.`,
  v2ImportKinds: {
    character: "캐릭터",
    module: "모듈",
    persona: "페르소나",
